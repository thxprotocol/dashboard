...
export default class ModalWidgetEdit extends Vue {
    ...
    get widgetEmbedCode() {
        ...
        return `<iframe width="${this.widget.metadata.width}" height="${this.widget.metadata.height}"
                                frameBorder="0" src="${this.widgetUrl}/?asset_pool=${this.pool.address}&client_id=${this.widget.clientId}&client_secret=${this.widget.clientSecret}&reward_id=${this.widget.metadata.rewardId}&reward_amount=${this.widget.reward.withdrawAmount}&reward_symbol=${this.pool.erc20.symbol}"></iframe>`;
    }
